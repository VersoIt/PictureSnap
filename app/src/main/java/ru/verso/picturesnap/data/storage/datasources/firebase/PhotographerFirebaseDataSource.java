...
public class PhotographerFirebaseDataSource implements PhotographerDataSource {
    ...
    public PhotographerFirebaseDataSource() {
    ...
    public void updateRatingOf(String photographerId, float lastAverage, int lastCount, int newAdd) {
        if (lastAverage == 0) {
            photographersReference.child(photographerId).child(Constants.FIREBASE_PHOTOGRAPHER_RATING_PATH).setValue(newAdd);
            return;
        }
